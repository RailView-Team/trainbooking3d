    ...
    const fetchTrains = async () => {
      ...
      try {
        ...
        const formattedTrains = backendTrains.map((train) => {
          // Generate realistic departure/arrival times based on train number
          const baseHour = (parseInt(train.trainNumber) % 24);
          const departureTime = `${String(baseHour).padStart(2, '0')}:${String((parseInt(train.trainNumber) % 60)).padStart(2, '0')}`;
          const arrivalHour = (baseHour + 8 + Math.random() * 4) % 24;
          const arrivalTime = `${String(Math.floor(arrivalHour)).padStart(2, '0')}:${String(Math.floor((arrivalHour % 1) * 60)).padStart(2, '0')}`;
